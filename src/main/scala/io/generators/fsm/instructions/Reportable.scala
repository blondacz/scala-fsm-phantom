...
implicit def instruction2Reportable[S <: V2Model.Instruction.MessageState,C <: V2Model.Instruction.ConfirmationState] : Reportable[(V2Model.Instruction[S,C],Seq[String])] = new Reportable[(V2Model.Instruction[S, C], Seq[String])] {
  override def report(i: (V2Model.Instruction[S, C], Seq[String])): String =   i._1.toString.padTo(20,' ') + " MessageState: " + i._1.ms.runtimeClass.getSimpleName.padTo(15,' ') + " ConfirmationState: " + i._1.cs.runtimeClass.getSimpleName.padTo(15,' ') + " Events: " + i._2.mkString("; ")
}
